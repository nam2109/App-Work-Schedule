                                      ...
                                      child: BarChart(
                                        BarChartData(
                                          ...
                                          barTouchData: BarTouchData(
                                            handleBuiltInTouches: false, // tắt mặc định
                                            touchCallback: (event, response) {
                                              if (event is FlTapUpEvent) { // chỉ nhận khi nhấc tay
                                                if (response?.spot != null) {
                                                  final idx = response!.spot!.touchedBarGroupIndex;
                                                  setState(() {
                                                    _selectedMonthIndex =
                                                        (_selectedMonthIndex == idx ? null : idx);
                                                  });
                                                } else {
                          ...
                          if (_selectedMonthIndex == null)
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(
                                  horizontal: 4.0),
                              child: Text(
                                'Đã chọn: ${DateFormat.yMMMM().format(months[_selectedMonthIndex!])} · ${byMonth[_ymKey(months[_selectedMonthIndex!])]?.length ?? 0} khóa',
